...
class App(QMainWindow):
    ...
    def show_index(self, idx: int):
        ...
        if sf.FileType.lower() == ".html":
            ...
            self.web.setHtml(f"<pre style='font-family:monospace;'>{content}</pre>")


def launch_viewer(subfiles: List[Subfile]):
    """Launch the ScriptCore viewer with a list of subfiles.
    
    Requires PySide6 to be installed (pip install PySide6).
    """
    if not HAS_PYSIDE6:
        raise ImportError(
            "PySide6 is required for the viewer. Install it with: pip install PySide6"
        )
